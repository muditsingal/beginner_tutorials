/**
 * @file test_tf2_broadcaster.cpp
 * @author Tommy Chang
          (Repo: https://github.com/TommyChangUMD/minimal-integration-test.git)
...
class TaskPlanningFixture : public testing::Test {
 public:
  ...
  void SetUp() override {
     ...
     */
    bool retVal =
        StartROSExec("minimal_integration_test", "minimal_publisher", "talker");
    ASSERT_TRUE(retVal);
 ...
 protected:
  ...
  bool StartROSExec(const char* pkg_name, const char* node_name,
    ...
    char execName[16];
    snprintf(execName, 16, "%s", exec_name);
...
TEST_F(TaskPlanningFixture, test_publisher_cnt) {
  std::cout << "Publisher count test beginning!!" << std::endl;
  using std_msgs::msg::String;
  using PUBLISHER = rclcpp::Publisher<String>::SharedPtr;
  PUBLISHER test_pub = node_->create_publisher<String>("talker_string", 5);
  auto pub_cnt = node_->count_publishers("talker_string");

  ASSERT_EQ(1, static_cast<int>(pub_cnt));
}
